...
export default function PoliticalTradesClient({ devSampleTrades = null }) {
  ...
  return (
    <div className="ptx-page">
      ...
      <CampaignFinanceSection members={members} onSelectMember={setSelected} />
...
/* ═══════════════════════ Campaign Finance (FEC / OpenFEC) ═══════════════════
   Additive section below the trades table. Reads the cached FEC routes
   (Supabase-first, honest empties). Reuses MiniAvatar / partyMeta / fmtUSD from
   this file — no forked primitives. Neutral presentation: amounts, mixes, donor
   industries only; source + cycle labeled on every card. */
...
function CampaignFinanceSection({ members, onSelectMember }) {
  const [cycle, setCycle] = useState(2026);

  // Build a bioguide→member index so a top-raiser row (which may not be in the
  // trades-derived set) can still open the shared member modal.
  ...
  const openMember = (row) => {
    ...
    // synthesize a minimal member object from the FEC row
    onSelectMember({
      name: row.name,
      party: row.party,
...
/* ── Campaign finance block for the member modal (FEC / OpenFEC). Fetches the
   ...
   mix, donor industries only. ── */
function MemberCampaignFinance({ member: m, cycle = 2026 }) {
  ...
  const sizeSplit = useMemo(() => {
    ...
    for (const b of buckets) {
      // FEC ScheduleABySize `size` is the lower bound of the bucket; <200 is the
      // unitemized/small-dollar band, the rest are itemized larger gifts.
    ...
    <div className="ptx-modal-backdrop" onClick={onClose}>
      ...
      >
        ...
        <div className="ptx-modal-right">
          ...
          <MemberCampaignFinance member={m} />
