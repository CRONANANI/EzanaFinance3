  ...
  async (request) => {
    try {
      ...
      const [positionsRes, tradesRes, activityRes, valueRes, tradedRes] = await Promise.all([
        fetch(
          `https://data-api.polymarket.com/positions?user=${resolvedWallet}&sizeThreshold=0.1`,
          { next: { revalidate: 30 } },
        ),
        fetch(`https://data-api.polymarket.com/trades?user=${resolvedWallet}&limit=100`, {
          next: { revalidate: 30 },
        }),
        fetch(`https://data-api.polymarket.com/activity?user=${resolvedWallet}&limit=50`, {
          next: { revalidate: 30 },
        }),
        fetch(`https://data-api.polymarket.com/value?user=${resolvedWallet}`, {
          next: { revalidate: 60 },
        }),
        fetch(`https://data-api.polymarket.com/traded?user=${resolvedWallet}`, {
          next: { revalidate: 300 },
        }),
      ]);
      ...
      const totalPortfolioValue =
        Array.isArray(valueData) && valueData[0]?.value ? Number(valueData[0].value) : 0;
      ...
      positionsArr.forEach((p) => {
        ...
        let cat = 'other';
        if (
          /president|election|congress|trump|biden|democrat|republican|vote|governor|senate/.test(
            title,
          )
        )
          cat = 'politics';
        else if (/bitcoin|ethereum|crypto|btc|eth|solana|defi/.test(title)) cat = 'crypto';
        else if (/rate|gdp|recession|fed|inflation|s&p|market|economy|tariff/.test(title))
          cat = 'economics';
        else if (/ai|tesla|apple|spacex|tech|openai|google|nvidia|microsoft/.test(title))
          cat = 'tech';
        else if (/nba|nfl|mlb|soccer|sport|super bowl|champion|world cup/.test(title))
          cat = 'sports';
        else cat = 'culture';
