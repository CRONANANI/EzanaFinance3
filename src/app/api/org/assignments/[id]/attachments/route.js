...
async function loadAssignment(supabase, orgId, assignmentId) {
...
/* GET /api/org/assignments/[id]/attachments — metadata rows (RLS-scoped) each
   with a short-lived signed download URL. */
export const GET = withApiGuard(
  async (_request, _user, context) => {
    ...
    const assignment = await loadAssignment(supabase, member.org_id, id);
    if (!assignment) return NextResponse.json({ error: 'Not found' }, { status: 404 });

    const { data: rows, error } = await supabase
      .from('org_assignment_attachments')
      .select(
        'id, assignment_id, submission_id, org_id, file_name, storage_path, size_bytes, uploaded_by, created_at',
      )
...
/* POST /api/org/assignments/[id]/attachments — multipart file upload. Uploads
   under {org_id}/assignments/{assignmentId}/{uuid}-{safeName} with the member's
   own (RLS-gated) client, then records the metadata row. */
export const POST = withApiGuard(
  async (request, _user, context) => {
    ...
    const submissionId = form.get('submission_id') || null;
    const path = `${member.org_id}/assignments/${id}/${crypto.randomUUID()}-${safeName}`;
...
/* DELETE /api/org/assignments/[id]/attachments?attachmentId=… — removes the
