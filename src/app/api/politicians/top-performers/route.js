...
async function handleGet(request) {
  if (!isServerSupabaseConfigured()) {
    console.error('[top-performers] missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY');
    return NextResponse.json(
      {
    ...
    const performers = Array.from(rollup.values())
      .map((r) => ({
        ...
        estimated_return_pct:
          r.total_volume_estimated > 0 ? (r.estimated_pnl / r.total_volume_estimated) * 100 : 0,
      }))
  ...
  if (!Number.isFinite(year) || year < 2000 || year > 2100) {
    return NextResponse.json({ error: 'year must be a 4-digit year or "all"' }, { status: 400 });
  }

  // Apply filters before order/limit so PostgREST builds a single correct query.
  let q = supabase.from('politician_annual_performance').select('*').eq('year', year);
  if (chamber) q = q.eq('chamber', chamber);
