...
const CATEGORIES = [
  ...
  },
  {
    ...
    description: 'Manage your investments and linked accounts',
    icon: Wallet,
    articles: [
...
const FAQ_ITEMS = [
  {
    ...
    a: 'Go to the Dashboard and click "Connect Brokerage". You\'ll be guided through Plaid\'s secure linking flow. We support most major US brokerages including Fidelity, Schwab, TD Ameritrade, and more.',
  },
  {
    q: 'What is congressional trading data?',
    a: 'Ezana tracks publicly disclosed trades by members of Congress. This data is required by the STOCK Act and provides insight into how lawmakers invest based on their legislative work.',
  },
  {
    q: 'Can I export my portfolio data?',
    a: 'Yes. From your Dashboard, use the Report button to export your portfolio summary as CSV. Full export options are available in Account Settings.',
  },
  {
    q: 'How do I contact support?',
    a: 'Email us at support@ezana.world or use the Contact button in the footer. We typically respond within 24 hours on business days.',
  },
];
        ...
        <div className="mx-auto max-w-3xl text-center">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-xl border border-[rgba(16,185,129,0.2)] bg-[rgba(17,24,39,0.8)] py-4 pl-12 pr-4 text-white placeholder-[#6b7280] outline-none transition-colors focus:border-[#10b981] focus:ring-2 focus:ring-[#10b981]/20"
            />
            <kbd className="absolute right-4 top-1/2 hidden -translate-y-1/2 rounded border border-[rgba(16,185,129,0.2)] bg-[rgba(17,24,39,0.6)] px-2 py-1 text-xs text-[#9ca3af] md:inline">
              ⌘K
            </kbd>
          </div>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {CATEGORIES.map((cat) => {
            ...
            return (
              <Link
                key={cat.id}
                href={`/help-center/category/${cat.id}`}
                className="group rounded-xl border border-[rgba(16,185,129,0.1)] bg-[rgba(26,35,50,0.6)] p-6 transition-all hover:border-[#10b981]/30 hover:bg-[rgba(16,185,129,0.05)]"
              >
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-lg bg-[rgba(16,185,129,0.15)] text-[#10b981] transition-colors group-hover:bg-[rgba(16,185,129,0.25)]">
                  <Icon className="h-6 w-6" />
                </div>
                <h3 className="mb-2 font-semibold text-white group-hover:text-[#10b981]">
                  {cat.title}
                </h3>
      ...
      <section className="mx-auto max-w-3xl px-4 py-16">
        ...
        <div className="space-y-2">
          {FAQ_ITEMS.map((item, i) => (
            <div
              key={i}
              className="rounded-xl border border-[rgba(16,185,129,0.1)] bg-[rgba(26,35,50,0.4)] overflow-hidden"
            >
              <button
                onClick={() => setExpandedFaq(expandedFaq === i ? null : i)}
                className="flex w-full items-center justify-between gap-4 px-6 py-4 text-left text-white transition-colors hover:bg-[rgba(16,185,129,0.05)]"
              >
                <span className="font-medium">{item.q}</span>
                <ChevronRight
                  className={`h-5 w-5 flex-shrink-0 text-[#10b981] transition-transform ${
                    expandedFaq === i ? 'rotate-90' : ''
                  }`}
                />
              </button>
              {expandedFaq === i && (
                <div className="border-t border-[rgba(16,185,129,0.1)] px-6 py-4 text-[#9ca3af]">
                  {item.a}
                </div>
              )}
            </div>
      ...
      <section className="border-t border-[rgba(16,185,129,0.1)] bg-gradient-to-b from-[#161b22] to-[#0f1419] px-4 py-16">
        <div className="mx-auto max-w-2xl rounded-2xl border border-[rgba(16,185,129,0.2)] bg-[rgba(26,35,50,0.6)] p-8 text-center">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:justify-center">
            <Link
              href="mailto:support@ezana.world"
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#10b981] px-6 py-3 font-medium text-white transition-colors hover:bg-[#059669]"
            >
              Contact Support
            ...
            <Link
              href="/"
              className="inline-flex items-center justify-center gap-2 rounded-lg border border-[rgba(16,185,129,0.3)] px-6 py-3 font-medium text-[#10b981] transition-colors hover:bg-[rgba(16,185,129,0.1)]"
            >
              Back to Home
            </Link>
          </div>
