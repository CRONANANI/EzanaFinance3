...
const RECENT_POSTS = [
  {
    id: 1,
    text: 'Just opened a position in NVDA ahead of earnings. Heres my thesis...',
    likes: 124,
    comments: 38,
    shares: 15,
    time: '2 hours ago',
  ...
  {
    id: 2,
    text: 'Weekly portfolio update: +3.2% this week. Top mover was MSFT after the Azure numbers.',
    likes: 89,
    comments: 22,
    shares: 8,
    time: '1 day ago',
  ...
  {
    id: 3,
    text: 'New course dropping next week: Advanced Options Strategies. Pre-register now.',
    likes: 156,
    comments: 47,
    shares: 31,
    time: '3 days ago',
  ...
  {
    id: 4,
    text: 'Why Im bullish on energy stocks in 2026 — a contrarian take. Thread',
    likes: 201,
    comments: 63,
    shares: 42,
    time: '5 days ago',
...
const COPIER_MESSAGES = [
  {
    from: 'Alex M.',
    text: 'Hey, love the NVDA call. Whats your stop loss?',
    time: '30 min ago',
    unread: true,
  },
  {
    from: 'Sarah K.',
    text: 'Can you do a video on your position sizing strategy?',
    time: '2 hours ago',
    unread: true,
  },
  {
    from: 'Mark R.',
    text: 'Thanks for the dividend picks — already up 8%!',
    time: '1 day ago',
    unread: false,
  },
];
  ...
  return (
    <div className="ptr-page">
      <div className="ptr-page-header">
        ...
        <div className="ptr-tab-group">
          {[
            ...
            <button
              key={t.key}
              className={`ptr-tab ${activeTab === t.key ? 'active' : ''}`}
              onClick={() => setActiveTab(t.key)}
            >
              <i className={`bi ${t.icon}`} /> {t.label}
              {t.key === 'messages' && COPIER_MESSAGES.filter((m) => m.unread).length > 0 && (
                <span className="ptr-tab-badge">
                  {COPIER_MESSAGES.filter((m) => m.unread).length}
                </span>
              )}
            </button>
          ))}
      ...
      <div className="ptr-stats-row ptr-stats-compact">
        <div className="ptr-stat-mini">
          <span className="ptr-stat-mini-value">{ENGAGEMENT_STATS.followers}</span>
          <span className="ptr-stat-mini-label">
            Followers <span className="positive">+{ENGAGEMENT_STATS.followersChange}</span>
          </span>
        </div>
        <div className="ptr-stat-mini">
          <span className="ptr-stat-mini-value">{ENGAGEMENT_STATS.likes.toLocaleString()}</span>
          <span className="ptr-stat-mini-label">Total Likes</span>
        </div>
        <div className="ptr-stat-mini">
          <span className="ptr-stat-mini-value">{ENGAGEMENT_STATS.comments}</span>
          <span className="ptr-stat-mini-label">Comments</span>
        </div>
        <div className="ptr-stat-mini">
          <span className="ptr-stat-mini-value">{ENGAGEMENT_STATS.avgEngagement}%</span>
          <span className="ptr-stat-mini-label">Engagement Rate</span>
        </div>
      </div>
        ...
        <div className="ptr-row-2">
          <div className="ptr-card" style={{ flex: 1.5 }}>
            <div className="ptr-compose">
              <textarea
                className="ptr-compose-input"
                value={newPost}
                onChange={(e) => setNewPost(e.target.value)}
                placeholder="Share a market insight, trade idea, or update with your followers..."
                maxLength={MAX_CHARS}
              />
              <div className="ptr-compose-actions">
                <div className="ptr-compose-tools">
                  <EmojiPicker onPick={insertEmoji} anchorRef={emojiAnchorRef} />
                  <button className="ptr-compose-tool" title="Image">
                    <i className="bi bi-image" />
                  </button>
                  <button className="ptr-compose-tool" title="Poll">
                    <i className="bi bi-bar-chart-steps" />
                  </button>
                  <button className="ptr-compose-tool" title="Schedule">
                    <i className="bi bi-clock" />
                  </button>
                </div>
                <div className="ptr-compose-right">
                  <span className={`ptr-char-count ${charClass}`}>
                    {len}/{MAX_CHARS}
                  </span>
                  <button className="ptr-btn-primary" disabled={!newPost.trim()}>
                    Post
                  </button>
                </div>
              </div>
            ...
            {RECENT_POSTS.map((post) => (
              <div key={post.id} className="ptr-post">
                {post.pinned && (
                  <span className="ptr-post-pinned">
                ...
                <div className="ptr-post-meta">
                  ...
                  <div className="ptr-post-stats">
                    <span>
                      <i className="bi bi-heart" /> {post.likes}
                    </span>
                    <span>
                      <i className="bi bi-chat" /> {post.comments}
                    </span>
                    <span>
                      <i className="bi bi-share" /> {post.shares}
                    </span>
                  </div>
                </div>
              </div>
                ...
                {FOLLOWER_ACTIVITY.map((f, i) => (
                  <div key={i} className="ptr-activity-item">
                    ...
                    <div className="ptr-activity-body">
                      <span className="ptr-activity-text">
      ...
      {activeTab === 'analytics' && (
        <div className="ptr-analytics-grid">
          <div className="ptr-card ptr-analytics-card">
            ...
            <div className="ptr-analytics-sub">
              Avg. {Math.round(ENGAGEMENT_STATS.likes / ENGAGEMENT_STATS.posts)} likes per post
            </div>
          </div>
          <div className="ptr-card ptr-analytics-card">
              ...
              >
                ...
                >
                  <div
                    style={{
                      height: '100%',
                      width: `${totalAvailable ? (totalEarned / totalAvailable) * 100 : 0}%`,
          ...
          <div className="ptr-tab-group" style={{ marginBottom: '1.25rem' }}>
            {['All', ...Object.keys(badgeCategories)].map((cat) => (
              <button
                key={cat}
                type="button"
                className={`ptr-tab ${badgeFilter === cat ? 'active' : ''}`}
            ...
            <div className="ptr-badges-grid">
              {(badgeFilter === 'All'
                ...
                return (
                  <div
                    key={badge.id}
                    className="ptr-card ptr-badge-card"
                    style={{
                      display: 'flex',
                      flexDirection: 'column',
                      alignItems: 'center',
                  ...
                  >
                    ...
                    >
                      <i
                        className={`bi ${badge.badge_icon}`}
                        style={{ color: badge.earned ? badge.tier_color : '#4b5563' }}
                      />
                    </div>
                    <div
                      style={{
