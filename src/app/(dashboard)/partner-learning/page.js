...
const RECENT_REVIEWS = [
  {
    student: 'Alex M.',
    course: 'Options Trading Basics',
    rating: 5,
    text: "Best options course I've taken. The iron condor section alone was worth it.",
    time: '2 days ago',
  },
  {
    student: 'Sarah K.',
    course: 'Congressional Trade Signals',
    rating: 5,
    text: 'Eye-opening content. Changed how I look at insider trading data.',
    time: '5 days ago',
  },
  {
    student: 'David W.',
    course: 'Building a Dividend Portfolio',
    rating: 4,
    text: 'Great framework. Would love more on international dividend stocks.',
    time: '1 week ago',
  },
];
...
export default function PartnerLearningPage() {
  ...
  const fetchEchoArticles = useCallback(async () => {
    ...
    if (!token) return;
    const res = await fetch('/api/echo/articles?my=true', {
    ...
    <div className="ptr-page">
      <div className="ptr-page-header">
        ...
        <div className="ptr-page-header-right">
          ...
          <button className="ptr-btn-primary">
      ...
      <div className="ptr-stats-row ptr-stats-compact">
        <div className="ptr-stat-mini">
          <span className="ptr-stat-mini-value">{CONTENT_STATS.totalStudents}</span>
          <span className="ptr-stat-mini-label">Total Students</span>
        </div>
        <div className="ptr-stat-mini">
          <span className="ptr-stat-mini-value">
            ${CONTENT_STATS.totalRevenue.toLocaleString()}
          </span>
          <span className="ptr-stat-mini-label">Course Revenue</span>
        </div>
        <div className="ptr-stat-mini">
          <span className="ptr-stat-mini-value">{CONTENT_STATS.avgRating} ★</span>
          <span className="ptr-stat-mini-label">Avg. Rating</span>
        </div>
        <div className="ptr-stat-mini">
          <span className="ptr-stat-mini-value">{CONTENT_STATS.completionRate}%</span>
            ...
            <div className="ptr-course-list">
              {PUBLISHED_COURSES.map((c) => (
                <div key={c.id} className="ptr-course-item">
                  <div className="ptr-course-icon">
                    <i className="bi bi-journal-bookmark-fill" />
                  </div>
                  <div className="ptr-course-info">
                    <span className="ptr-course-title">{c.title}</span>
                    <span className="ptr-course-meta">
                      {c.modules} modules · Updated {c.lastUpdated}
                    </span>
                  </div>
                  <div className="ptr-course-stats-row">
                    <div className="ptr-course-stat">
                      <span className="ptr-course-stat-val">{c.students}</span>
                      <span className="ptr-course-stat-lbl">Students</span>
                    </div>
                    <div className="ptr-course-stat">
                      <span className="ptr-course-stat-val">${c.revenue.toLocaleString()}</span>
                      <span className="ptr-course-stat-lbl">Revenue</span>
                    </div>
                    <div className="ptr-course-stat">
                      <span className="ptr-course-stat-val">{c.rating} ★</span>
                      <span className="ptr-course-stat-lbl">Rating</span>
                    </div>
                  </div>
            ...
            <div className="ptr-course-list">
              {DRAFT_COURSES.map((c) => (
                <div key={c.id} className="ptr-course-item ptr-course-draft">
                  <div className="ptr-course-icon draft">
                    <i className="bi bi-pencil-square" />
                  </div>
                  <div className="ptr-course-info">
                    <span className="ptr-course-title">{c.title}</span>
                    <span className="ptr-course-meta">
                      {c.modules}/{c.modulesPlanned} modules complete
                    </span>
                  </div>
                  <div className="ptr-draft-progress">
                    <div className="ptr-draft-progress-bar">
                      <div
                        className="ptr-draft-progress-fill"
        ...
        <div className="ptr-card" style={{ marginBottom: '1.25rem' }}>
          <div className="ptr-card-header">
            ...
            <span className="ptr-card-count">
              {isApprovedWriter ? 'Write & publish articles' : 'Apply to become a writer'}
            </span>
          </div>
                    ...
                    {echoArticles.map((a) => (
                      ...
                      >
                        <div className="ptr-course-info">
                          ...
                          <span className="ptr-course-meta">
                            {a.article_category} ·{' '}
                            <span className={`echo-status-badge echo-status-${a.article_status}`}>
                              {a.article_status}
                            </span>
                          </span>
                        </div>
                        <button className="ptr-btn-sm" onClick={() => setEditingArticle(a)}>
                          Edit
                        </button>
                      </div>
                    ))}
            ...
            {PUBLISHED_COURSES.map((c) => (
              <div key={c.id} className="ptr-analytics-bar-row">
                ...
                <div className="ptr-analytics-bar-track">
                  <div
                    className="ptr-analytics-bar-fill"
                    style={{ width: `${(c.revenue / CONTENT_STATS.totalRevenue) * 100}%` }}
                  />
                </div>
          ...
          <div className="ptr-card ptr-analytics-card">
            ...
            {PUBLISHED_COURSES.map((c) => (
              <div key={c.id} className="ptr-analytics-bar-row">
                ...
                <div className="ptr-analytics-bar-track">
                  <div
                    className="ptr-analytics-bar-fill blue"
                    style={{ width: `${(c.students / CONTENT_STATS.totalStudents) * 100}%` }}
                  />
                </div>
