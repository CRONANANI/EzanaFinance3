...
/**
 * Error boundary for the org-trading segment. Renders OUTSIDE the OrgProvider
 ...
 */
export default function OrgTradingError({ error, reset }) {
  const isDev = process.env.NODE_ENV === 'development';

  useEffect(() => {
    // eslint-disable-next-line no-console
    console.error('[OrgTradingError]', error, { digest: error?.digest });
  }, [error]);
    ...
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-4 text-center">
      <div className="max-w-xl">
        <h2 className="text-2xl font-bold text-foreground mb-2">
          We couldn&apos;t load Council Trading
        </h2>
        <p className="text-muted-foreground mb-4">
          {isDev
            ? 'A render-time error was caught below. Check the browser console for the full stack.'
            : 'Something went wrong loading the trading desk. Try again, or head back to the Command Center.'}
        </p>
