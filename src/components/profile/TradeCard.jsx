        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {trade.status === 'open' ? (
              ...
              <span className="rounded bg-zinc-500/20 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-zinc-400">
          ...
          <p className="mt-2 text-xs text-[#6b7280]">
            Risk: <span className="capitalize text-[#9ca3af]">{trade.risk_level || 'moderate'}</span>
          ...
          {(trade.tags || []).length > 0 && (
            <p className="mt-1 truncate text-[10px] uppercase tracking-wider text-[#6b7280]">
              {(trade.tags || [])[0]}
            </p>
          )}
        </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-4 border-t border-[#1a1a24] pt-3 text-xs text-[#6b7280]">
        ...
        {isOwner && (
          <div className="relative ml-auto" ref={ref}>
            <button
              type="button"
              className="rounded px-2 py-1 text-[#9ca3af] hover:bg-[#1a1a24]"
            ...
            {menu && (
              <div className="absolute bottom-full right-0 z-20 mb-1 min-w-[140px] rounded-lg border border-[#1a1a24] bg-[#0d0d14] py-1 shadow-xl">
                {['Partial Exit', 'Full Exit', 'Average Down'].map((label) => (
                  <button
                    key={label}
                    type="button"
                    className="block w-full px-3 py-2 text-left text-xs text-[#e5e7eb] hover:bg-[#1a1a24]"
