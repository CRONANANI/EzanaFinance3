...
/*
 * Learning Center ΓÇö contrast tokens (see learning-center.css ΓåÆ .lc-surface-*)
 * ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
 * Every card on this page wraps content in <LearningCard surface=...>.
 * Inside, use the helpers ΓÇö never inline hex colors:
 *
 *   <CardPrimary>    ΓåÆ important text (titles, values)
 *   <CardSecondary>  ΓåÆ body / label text
 *   <CardMuted>      ΓåÆ meta / timestamps / secondary captions
 *   <CardBadge>      ΓåÆ pill-style status labels
 *
 * Surface ΓåÆ intent:
 *   default    ΓÇö regular page cards
 *   tinted     ΓÇö emerald-washed cards (journey, track tiles)
 *   accent     ΓÇö fully branded (emerald bg, white fg) ΓÇö selected track
 *   muted      ΓÇö secondary-tier cards
 *   completed  ΓÇö completed course rows (green wash)
 *   locked     ΓÇö locked course rows (dim but still readable)
 *
 * Raw color tokens (when CSS class isn't enough):
 *   var(--text-primary)    ΓåÆ headings / values
 *   var(--text-secondary)  ΓåÆ body
 *   var(--text-muted)      ΓåÆ de-emphasised
 *   var(--text-faint)      ΓåÆ captions, timestamps (use sparingly)
 *   var(--emerald)         ΓåÆ brand accent
 *   var(--emerald-text)    ΓåÆ brand text (flips darker on light)
 */
...
export function LearningCenterPage() {
  ...
  // Dev-only contrast audit ΓÇö logs any text on this page that fails WCAG AA
    ...
    >
      <div className="lc3-header-row">
        <div>
          ...
          >
            Master the markets ΓÇö from beginner to expert
          </p>
        </div>
      </div>
      ...
      {/* ΓöÇΓöÇ Your Learning Journey (tinted surface) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <LearningCard
        as="div"
        surface="tinted"
        className="db-card"
      ...
      >
        ...
        >
          <CardPrimary as="h2" style={{ fontSize: '.9375rem', fontWeight: 800, margin: 0 }}>
        ...
        <div className="lc3-journey-grid">
          <div>
            ...
            {inProgressCourses.length === 0 ? (
              <CardMuted as="p" style={{ fontSize: '.8125rem', margin: '.25rem 0 0' }}>
                ...
                return (
                  <div key={c.id} className="lc3-inprogress-row">
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <CardMuted as="p" style={{ fontSize: '.6875rem', margin: 0 }}>
          ...
          <div>
            ...
            {suggestedCourses.length === 0 ? (
              <CardMuted as="p" style={{ fontSize: '.8125rem', margin: '.25rem 0 0' }}>
                You&apos;re all caught up ΓÇö explore tracks below.
              </CardMuted>
            ) : (
      ...
      {/* ΓöÇΓöÇ Org-assigned learning (default surface, purple-accent trim) ΓöÇΓöÇ */}
      {isOrgUser && (
        <LearningCard
          as="div"
          surface="default"
          ...
          >
            {[
              ...
              >
                ...
                >
                  <CardBadge tone={a.status === 'in_progress' ? 'info' : 'neutral'}>
                    {a.status.replace('_', ' ')}
                  </CardBadge>
      ...
      {/* ΓöÇΓöÇ Track progress tiles (tinted / accent when selected) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      ...
      {/* ΓöÇΓöÇ Friends + course panel ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      <div className="lc3-bottom-grid">
        <FriendsLearningCard />

        <LearningCard as="div" surface="default" className="db-card lc3-course-panel">
          <div className="db-card-header lc3-course-panel-header">
            <div>
              <CardPrimary as="h3" style={{ margin: 0, fontSize: '1rem', fontWeight: 800 }}>
                {TRACK_ICONS[selectedTrack]}{' '}
                {tracks.find((t) => t.id === selectedTrack)?.shortLabel || 'Track'}
              </CardPrimary>
              <CardMuted as="p" style={{ fontSize: '.75rem', marginTop: '.15rem' }}>
            ...
            <div className="lc3-level-tabs" role="tablist">
              {LEVEL_KEYS.map((lv) => (
                <button
                  ...
                  className={
                    selectedLevel === lv ? 'lc3-level-tab lc3-level-tab--active' : 'lc3-level-tab'
                  }
                >
                  {getLevelLabel(lv)}
                </button>
          ...
          <div style={{ padding: '0 1.25rem 1rem' }}>
            {!levelUnlocked ? (
              <CardMuted as="p" style={{ fontSize: '.8125rem', padding: '.75rem 0' }}>
                Complete all {getLevelLabel(prevLevelKey)} courses in this track to unlock{' '}
...
/**
 * Course row ΓÇö surface adapts to course state so the status badge, title
...
function CourseRow({ course, index, progress, access }) {
  ...
  const inProgress = progress && progress.status === 'in_progress' && !completed;
  const pct = progress?.progress_pct || 0;
  const locked = !access.ok;

  const surface = completed ? 'completed' : locked ? 'locked' : inProgress ? 'tinted' : 'default';
  ...
  const body = (
    <>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <CardPrimary as="p" style={{ fontSize: '.8125rem', fontWeight: 600, margin: 0 }}>
          {course.title}
        </CardPrimary>
        <CardMuted as="p" style={{ fontSize: '.6875rem', margin: 0 }}>
  ...
  return (
    <LearningCard as="div" surface={surface} className={rowClass} aria-disabled>
      {body}
    </LearningCard>
  );
}
