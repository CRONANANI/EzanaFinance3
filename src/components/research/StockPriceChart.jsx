...
export default function StockPriceChart({ symbol, livePrice = null }) {
  ...
  const isPositive =
    lastPrice != null && firstPrice != null ? lastPrice >= firstPrice : true;
  const pctChange =
  ...
  const minPrice = candles.length
    ? Math.min(...candles.map((c) => (c.low ?? c.price) * 0.998))
    : 0;
  ...
  return (
    ...
    >
      ...
      {/* ── Footer stats: Open / High / Low / Volume ── */}
      {candles.length > 0 &&
        (() => {
          const last = candles[candles.length - 1];
          const allH = candles.map((c) => c.high ?? c.price);
          const allL = candles.map((c) => c.low ?? c.price);
          const period_high = Math.max(...allH);
          const period_low = Math.min(...allL);
          ...
          return (
            ...
            >
              {[
                ...
                {
                  label: 'High',
                  value: `$${period_high.toFixed(2)}`,
                  colour: '#10b981',
                },
                {
                  label: 'Low',
                  value: `$${period_low.toFixed(2)}`,
                  colour: '#ef4444',
                },
                {
                  label: 'Close',
                  value: `$${Number(last.price).toFixed(2)}`,
                },
                {
                  label: 'Volume',
                  value:
