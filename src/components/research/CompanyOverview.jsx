  ...
  if (error || !data) {
    return (
      <motion.div
        ...
        className="research-card bg-[#0d1117] border border-gray-700 rounded-xl p-6"
      >
        <p className="text-gray-400">Unable to load company profile.</p>
      </motion.div>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        {data.logo && (
          <img src={data.logo} alt="" className="w-14 h-14 rounded-lg object-contain bg-[#161b22]" />
        )}
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-semibold text-white">{data.name || symbol}</h3>
          ...
          <div className="mt-4 flex flex-wrap gap-4">
            <div>
              ...
              <p className="text-white font-medium">{formatMarketCap(data.marketCapitalization)}</p>
            </div>
            {data.ipo && (
              <div>
                <span className="text-xs text-gray-500">IPO</span>
                <p className="text-white font-medium">{data.ipo}</p>
              </div>
            )}
          </div>
        </div>
