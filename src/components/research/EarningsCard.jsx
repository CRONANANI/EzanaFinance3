    ...
    return (
      <div className="research-fade-in research-card bg-white dark:bg-[#0d1117] border border-gray-200 dark:border-gray-700 rounded-xl p-6">
        <p className="text-gray-500 dark:text-gray-400">
          Earnings data unavailable (free tier: last 4 quarters).
        </p>
      </div>
    );
  }
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {data.map((row) => (
                <tr
                  key={`${row.period}-${row.year}`}
                  className="border-b border-gray-200/80 dark:border-gray-700/50"
                >
                  ...
                  <td
                    className={`py-2 ${Number(row.surprisePercent) >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}
                  >
