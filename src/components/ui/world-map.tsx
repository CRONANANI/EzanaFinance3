...
type WorldMapProps = {
  ...
  onPowerCountryClick?: (p: { iso: string; name: string; lng: number; lat: number }) => void;
  /**
...
export const WorldMap = forwardRef<WorldMapHandle, WorldMapProps>(function WorldMap(
  ...
  const { map, svgMap, width, height, baseDots } = useMemo(() => {
    const m = new DottedMap({ height: 170, grid: 'diagonal' });
  ...
  const scoreByIso = useMemo(() => {
    const o: Record<string, { color: string; opacity: number; glow: boolean; score: number }> = {};
      ...
      try {
        ...
        for (const f of fc.features) {
          ...
          const nameFromProp =
            ...
            iso;
          const centroid = featureCentroid(p, f.geometry as { type: string; coordinates: unknown });
          out.push({
            iso,
            name: nameFromProp,
            paths,
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (e.touches.length === 1 && isDragging) {
        ...
        const d = Math.hypot(
          t.clientX - panPointerStartRef.current.ex,
          t.clientY - panPointerStartRef.current.ey,
        );
        if (d > 6) dragMovedRef.current = true;
        setTransform((prev) => ({
          ...prev,
  ...
  const getDotColor = (center: FinancialCenter): string => {
    ...
    if (activeLayer === 'indices') {
      ...
      const indexPerformance: Record<string, number> = {
        ...
        Sydney: 0.45, // ASX up
        'São Paulo': 1.2, // Bovespa up
        Toronto: -0.12, // TSX down
        'Mexico City': -0.45, // IPC down
        'Buenos Aires': 2.1, // Merval up
        Santiago: -0.15, // SSE down
        Moscow: -1.5, // MOEX down
  ...
  return (
    ...
    >
      {!hideControls && (
        ...
        >
          <button type="button" onClick={() => zoomTowardCenter(1.3)} title="Zoom in">
            +
          </button>
          <button type="button" onClick={() => zoomTowardCenter(1 / 1.3)} title="Zoom out">
            −
          </button>
          ...
          >
            ...
            {isPowerMapActive && dotIsos
              ? baseDots.map((d, i) => {
                  ...
                  // Radii scaled down by ~0.82 (0.18/0.22) to match the higher
                  // 170-row grid resolution so dots stay crisp and separated.
                  const radius = data.score >= 90 ? 0.26 : data.score >= 70 ? 0.21 : 0.18;
