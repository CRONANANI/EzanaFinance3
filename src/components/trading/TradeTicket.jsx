      ...
      fetch('/api/notifications/track', {
        ...
        body: JSON.stringify({
          ...
          event_data: {
            ...
            side,
            qty: amountType === 'dollars' ? (data.notional ?? qty) : (data.qty ?? qty),
            type: orderType,
          },
        }),
      }).catch(() => {});
    ...
    <>
        ...
        <div className="trd-confirm-overlay" onClick={() => setShowConfirm(false)}>
          <div className="trd-confirm-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="trd-confirm-summary">
              <p>
                <strong>{side === 'buy' ? 'Buy' : 'Sell'}</strong> {selectedAsset?.symbol}
              </p>
              <p>
                {amountType === 'shares' ? `${qty} shares` : `$${qty}`} @{' '}
                {orderType === 'market' ? 'Market' : `Limit $${limitPrice}`}
              </p>
            </div>
            <div className="trd-confirm-actions">
              <button className="trd-btn-secondary" onClick={() => setShowConfirm(false)}>
                Cancel
              </button>
              <button
                className={`trd-btn-primary trd-submit-${side}`}
                onClick={doSubmit}
                disabled={loading}
              >
      ...
      <div className="trd-form-card trd-ticket">
        ...
        <div className="trd-side-toggle">
          <button
            className={`trd-side-btn buy ${side === 'buy' ? 'active' : ''}`}
            onClick={() => setSide('buy')}
          >
            Buy
          </button>
          <button
            className={`trd-side-btn sell ${side === 'sell' ? 'active' : ''}`}
        ...
        <div className="trd-form-body">
          <div className="trd-field" style={{ position: 'relative' }}>
            ...
            <input
              ...
              placeholder="Search AAPL, NVDA, SPY..."
            />
          ...
          {selectedAsset && (
            <>
              ...
              <div className="trd-field">
                ...
                <div className="trd-option-row">
                  {['market', 'limit'].map((t) => (
                    <button
                      key={t}
                      className={`trd-option-btn ${orderType === t ? 'active' : ''}`}
              ...
              <div className="trd-field">
                ...
                <div className="trd-option-row">
                  <button
                    className={`trd-option-btn ${amountType === 'shares' ? 'active' : ''}`}
                  ...
                  {selectedAsset.fractionable && (
                    <button
                      className={`trd-option-btn ${amountType === 'dollars' ? 'active' : ''}`}
              ...
              <div className="trd-field">
                ...
                <div className="trd-amount-input-wrap">
                  ...
                  <input
                    className="trd-input trd-amount-input"
                    type="number"
                    value={qty}
                    onChange={(e) => setQty(e.target.value)}
                    placeholder={amountType === 'shares' ? '1' : '50.00'}
                    min="0"
                    step={amountType === 'shares' ? '1' : '0.01'}
                  />
                </div>
              </div>
              {orderType === 'limit' && (
              ...
              <div className="trd-field">
                ...
                <div className="trd-option-row">
                  {[
                    { v: 'day', l: 'Day' },
                    { v: 'gtc', l: 'Good til Canceled' },
                  ].map((t) => (
                    <button
                      key={t.v}
                      className={`trd-option-btn ${timeInForce === t.v ? 'active' : ''}`}
                      onClick={() => setTimeInForce(t.v)}
                    >
                      {t.l}
                    </button>
                  ))}
              ...
              <button
                ...
                disabled={!qty || loading || parseFloat(qty) <= 0}
              >
                {loading
                  ? 'Submitting...'
