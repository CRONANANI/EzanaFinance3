-- ═══════════════════════════════════════════════════════════
-- FIX: Missing profiles columns + Isabel full-access setup
--
-- Safe to re-run. Catches up live DB when stripe/partner migrations
-- were never applied, then runs Isabel setup on top.
-- PREREQUISITE: isabel.lim546@gmail.com must exist in auth.users.
-- ═══════════════════════════════════════════════════════════

-- STEP 1: Catch up profiles table with all required columns
...
-- STEP 2: Ensure partners table exists with expected shape
...
-- STEP 3: Ensure organizations + org_members tables exist
...
-- STEP 4: Isabel full-access setup (now safe to run)
...
BEGIN
  ...
  INSERT INTO public.partners
    (user_id, username, display_name, status, bio, specializations, partner_type)
  VALUES (
    _uid, 'isabel.lim', 'Isabel Lim', 'active',
    'Full-access account with partner/creator privileges.',
    ARRAY['markets', 'analysis', 'education'], 'creator'
  )
