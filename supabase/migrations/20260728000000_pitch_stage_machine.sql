...
CREATE INDEX IF NOT EXISTS idx_pitch_discussion_open
  ON public.org_pitch_discussion_messages(pitch_id, status)
  WHERE status = 'open';
...
CREATE TABLE IF NOT EXISTS public.org_pitch_model (
  ...
  pitch_id      uuid NOT NULL REFERENCES public.org_pitches(id) ON DELETE CASCADE,
  org_id        uuid NOT NULL REFERENCES public.organizations(id) ON DELETE CASCADE,
  kind          text NOT NULL CHECK (kind IN ('three_statement', 'dcf', 'comps', 'sensitivity', 'other')),
  ref           text,        -- storage path | url | note id
...
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.decision = 'object' AND (NEW.reason IS NULL OR btrim(NEW.reason) = '') THEN
    RAISE EXCEPTION 'A cross-desk objection requires a reason.';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
